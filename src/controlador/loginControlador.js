...
router.post('/iniciarS', async function(req, res, next) {
    ...
    if(uexiste === " " || uexiste === null){
        return res.status(500).send({estado: 'Fallo', mensaje:'Usuario no existe o contrasenia incorrecta'});
    }
    //console.log(pexiste);
    ...
    if(token !== undefined && token !== ""){
        ...
        if(verificacion.mensaje === 'correcto' && verificacion !== {}){
            ...
            if(estado === " " || estado === null){
                return res.status(500).send({estado: 'Fallo', mensaje:'No se cerro sesion'});
            }
